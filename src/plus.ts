...
class Page {
  ...
  private wrap(t: string, mw: number, s: number, fontName: string): string[] {
    const words = t.split(' ');
    const lines: string[] = [];
    let cur = ''
      ...
      if (measure(test, s, fontName) <= mw) {
        ...
        if (measure(w, s, fontName) <= mw) {
          ...
          for (const char of w) {
            ...
            if (measure(charTest, s, fontName) <= mw) {
              temp = charTest;
            } else {
