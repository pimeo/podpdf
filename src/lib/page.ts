...
export class Page {
    ...
    table(data: string[][], x: number, y: number, o: TableOpts) {
        ...
        for (let i = 0; i < o.columns.length; i++) {
            const c = o.columns[i]
            // We pass maxWidth to support wrapping in headers too
            this.text(c.header, cx + (c.align === 'center' ? cw[i] / 2 : c.align === 'right' ? cw[i] - p : p), cy + p + fs * 0.8, { size: fs, weight: 'bold', color: o.headerColor ?? '#000', align: c.align, maxWidth: cw[i] - p * 2 })
            cx += cw[i]
        }
        cy += hh
        ...
        for (let r = 0; r < data.length; r++) {
            ...
            for (let i = 0; i < o.columns.length; i++) {
                const c = o.columns[i]
                this.text(data[r][i] ?? '', cx + (c.align === 'center' ? cw[i] / 2 : c.align === 'right' ? cw[i] - p : p), cy + p + fs * 0.8, { size: fs, align: c.align, maxWidth: cw[i] - p * 2 })
                cx += cw[i]
            }
            cy += rh
        }
        ...
        this.rect(x, y, tw, cy - y, { stroke: o.borderColor ?? '#CCC' })
        return this
    }
